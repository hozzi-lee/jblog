				...
				<form action="${pageContext.request.contextPath}/${authUser.id}/admin/write" method="post">
			      	<table id="admin-write">
			      		...
			      		<tr>
				      		...
				      		<td>
				      			<select id="selectCateInfo" name="cateNo">
				      				<!-- 카테고리 리스트 영역 -->
				      				<%-- <option value="">카테고리리스트</option> --%>
				      				<!-- 카테고리 리스트 영역 -->
				      			</select>
				      		</td>
			      		</tr>
...
function fetchList(){
	$.ajax({
		url: "${pageContext.request.contextPath}/{authUser.id}/admin/writeForm/cateOptionList",
		type: "post",
		
		success: function(returnCateInfo) {
			for (var i = 0; i < returnCateInfo.length; i++) {
				render(returnCateInfo[i]);
			};
...
function render(returnCateInfo){
	var str = '<option value="' + returnCateInfo.cateNo + '">' + returnCateInfo.cateName + '</option>'
	
	$('#selectCateInfo').append(str);
};
